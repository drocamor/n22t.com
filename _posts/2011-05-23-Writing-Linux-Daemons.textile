...
# Change it's working directory to /
# Set the umask to 0
# Redirect stdin, stdout and stderr to a logfile or /dev/null
# Write it's PID to a pidfile that it's init script references (This is probably not a requirement of all daemons, but if you're working with Debian GNU/Linux and want to use the skeleton init scripts without much work you should do this)
